...
<?php
    ...
    $threads = mysqli_fetch_assoc($result);
?>
...
<?php if ($threads): ?>
    ...
    <div class="container">
        <div class="jumbotron-custom">
            <h1 class="display-6 fw-bold mb-3"><?= htmlspecialchars($threads['thread_title']) ?></h1>
            <p class="lead"><?= htmlspecialchars($threads['thread_desc']) ?></p>
            ...
            <p class="my-4"><b>Posted by: Lalit</b></p>
        </div>
    </div>
<?php endif; ?>
...
<div class="container mb-5">
    <h2 class="page-header">Thread Discussion</h2>

    <!-- Search Bar -->
    <div class="search-bar">
        <input type="text" class="form-control mb-4" placeholder="Search comments...">
    </div>
    ...
    <div class="row gy-4">
        <?php
            $commentSql = "SELECT * FROM `comments` WHERE `comment_thr_id` = '$thread_id';";
            ...
            if (mysqli_num_rows($commentResult) > 0):
                ...
                <div class='thread-card d-flex align-items-start'>
                    ...
                    <div>
                        <h5 class='thread-title mb-1'>{$comment['comment_title']}</h5>
                        <p class='thread-description mb-2'>{$comment['comment_desc']}</p>
        ...
        ?>
        <p class="text-center">No Discussions found in this thread. Be the first to discuss!</p>
        <?php endif; ?>
    </div>
    <!-- Floating Ask Question Button -->
    <button href="#" class="ask-btn"  data-bs-toggle="modal" data-bs-target="#commentModal">Post Comment</button>
...
<?php include "partials/_commentModal.php"; ?>
